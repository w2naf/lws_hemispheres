...
def my_xticks(sDate,eDate,ax,radar_ax=False,labels=True,short_labels=False,
                fmt='%d %b',fontdict=None,plot_axvline=True,zorder=5000):
    if fontdict is None:
        ...
        if curr_date.day in doms:
            ...
            if labels:
                ...
                ax.text(xpos,ypos,txt,transform=ytransaxes,
                        ...
                        fontdict=fontdict,zorder=zorder)
            if short_labels:    
                if curr_date.day == 1:
                    ypos    = -0.030
                    ...
                    ax.text(xpos,ypos,txt,transform=ytransaxes,
                            ha='left', va='top',rotation=0,
                            fontdict=fontdict,zorder=zorder)
                    ax.axvline(xpos,lw=2,zorder=zorder,color='0.6',ls='--')
        curr_date += datetime.timedelta(days=1)
...
#    gw_arrows.append({'height':0.40,'color':'blue',   'date':datetime.datetime(2018,12,25)})
    ...
    gw_arrows.append({'height':0.40,'color':'blue',   'date':datetime.datetime(2019,3,1)})
...
def stackplot(po_dct,params,season,radars=None,sDate=None,eDate=None,
    ...
    for inx,param in enumerate(params):
        ...
        ax.set_title('')
        ax.set_title(txt,fontdict=left_title_fontdict,loc='left',zorder=1000)
